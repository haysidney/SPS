...
WaitPixelColors(p_DesiredColors,p_PosX,p_PosY,p_TimeOut=0,p_GetMode="",p_ReturnColor=0) {
    l_Start := A_TickCount
    Loop {
        PixelGetColor, l_OutputColor, %p_PosX%, %p_PosY%, %p_GetMode%
        If ( ErrorLevel )
        ...
        for index, desiredColor in p_DesiredColors
        {
            If ( l_OutputColor = desiredColor )
                Return ( p_ReturnColor ? l_OutputColor : 0 )
        }
